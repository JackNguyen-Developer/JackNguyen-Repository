...
public class MusicManager {
	...
	private static String songTitlePlaying;// musicTitle
	private static String albumArt;// image album
	//
	private static ArrayList<String> arrayPlay = new ArrayList<String>();
	private static ArrayList<String> arraySelect = new ArrayList<String>();

	private static boolean isRepeat = false;
	private static boolean isShuffle = false;
	private static boolean isPlaying = false;
	private static boolean isPlayMusicWithArraySelect = false;
	...
	public void addArraySelect(String item)
	{
		this.arraySelect.add(item);
	}
	public void resetArraySelect()
	{
	...
	public boolean playButton() {
		try {
			...
			{
				if (arrayPlay.size() > 0 && isPlaying || arraySelect.size() > 0
						&& isPlaying) {
	...
	/*
	 * public ArrayList<Song> getPlayList() { ArrayList<Song> arraySong = new
	 * ArrayList<Song>(); Cursor cur = getAllSong(); cur.moveToFirst(); String
	 * title, path = null, album = null, artist = null; long id = -1; while
	 * (!cur.isAfterLast()) { try { id = cur.getLong(0); title =
	 * cur.getString(1).toString(); path = cur.getString(2).toString(); album =
	 * cur.getString(3).toString(); artist = cur.getString(4).toString(); Song
	 * song = new Song(); song.setId(id); song.setTitle(title);
	 * song.setPath(path); song.setAlbum(album); song.setArtist(artist);
	 * arraySong.add(song); break; } catch (Exception e) { Log.e("",
	 * "error MusicManager.getPlayList" + e.toString()); } } cur.close(); return
	 * arraySong; }
	 */
	...
	public Cursor getListMusic(String select, String titleChild, long idChild) {
		...
		if (select.equalsIgnoreCase("All")) {
			...
			cursor = cr.query(MediaStore.Audio.Genres.Members.getContentUri(
					"external", idChild), projection, null, null,
					MediaStore.Audio.Media.DEFAULT_SORT_ORDER);
			...
			cursor = cr.query(MediaStore.Audio.Playlists.Members.getContentUri(
					"external", idChild), projection, null, null,
	...
	public Cursor getAlbumMusic() {
		// ContentResolver cr = context.getContentResolver();
		String[] arrayAlbum = { "DISTINCT " + MediaStore.Audio.Albums.ALBUM,
	...
	public Cursor getArtistMusic() {
		// ContentResolver cr = context.getContentResolver();
		String[] arrayAlbum = { "DISTINCT " + MediaStore.Audio.Artists.ARTIST,
				MediaStore.Audio.Artists._ID };
		Cursor cur = cr.query(MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI,
				arrayAlbum, null, null,
	...
	public Cursor getGenreMusic() {
		// ContentResolver cr = context.getContentResolver();
		String[] arrayAlbum = { "DISTINCT " + MediaStore.Audio.Genres.NAME,
				MediaStore.Audio.Genres._ID };
		Cursor cur = cr.query(MediaStore.Audio.Genres.EXTERNAL_CONTENT_URI,
				arrayAlbum, null, null,
	...
	public Cursor getPlayListMusic() {
		// ContentResolver cr = context.getContentResolver();
		String[] arrayAlbum = { "DISTINCT " + MediaStore.Audio.Playlists.NAME,
	...
	public Cursor getTypeMusic() {
		String[] arrayAlbum = { "DISTINCT " + MediaStore.Audio.Media.MIME_TYPE/*
																			 * ,
																			 * MediaStore
																			 * .
																			 * Audio
	...
	public void addPlayList(String name) {
		...
		cv.put(MediaStore.Audio.Playlists.DATE_MODIFIED,
				System.currentTimeMillis());
		cv.put(MediaStore.Audio.Playlists.NAME, name);
		// cv.put(MediaStore.Audio.Playlists._ID, id);
		// Uri uri = MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI;
		Uri uri = cr
				.insert(MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, cv);
		Log.e("", "Add new Playlist: " + uri);
	}

	public void addItemPlayList(long idPlayList, ArrayList<String> arrayMusic,
			String titleMusic) {
		if (arrayMusic != null && titleMusic == null)/*
													 * .size() > 0 && titleMusic
													 * == null)
													 */
		{
		...
		/*
		 * String title = titleMusic; ContentValues cv = new ContentValues();
		 * cv.put(MediaStore.Audio.Playlists.Members.PLAY_ORDER, 2);
		 * cv.put(MediaStore.Audio.Playlists.Members.AUDIO_ID, idSong); Uri uri
		 * =
		 * cr.insert(MediaStore.Audio.Playlists.Members.getContentUri("external"
		 * , idPlayList), cv);
		 */
	...
	private void insertItemPlayList(long idPlayList, long idSong) {
		...
		Uri uri = cr.insert(MediaStore.Audio.Playlists.Members.getContentUri(
				"external", idPlayList), cv);
	}
}
